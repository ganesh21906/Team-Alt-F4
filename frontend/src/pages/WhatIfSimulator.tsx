...
export function WhatIfSimulator() {
  ...
  const [input, setInput] = useState<WhatIfInput>({ attendancePct: 85, assessmentScore: 75, assignmentScore: 70 });
  ...
  // Load student baseline on mount
  useEffect(() => {
    let active = true;
    const loadBaseline = async () => {
      try {
        const profile = await api.getStudentProfile(studentLevel, studentId);
        if (active) {
          setInput({
            attendancePct: profile.attendancePct || 85,
            assessmentScore: profile.subjects[0]?.currentScore || 75,
            assignmentScore: profile.subjects[1]?.currentScore || 70
          });
          setIsInitialized(true);
        }
      } catch (err) {
        console.error("Failed to load student baseline", err);
        if (active) {
          setIsInitialized(true);
        }
      }
  ...
  // Run What-If ML prediction on inputs change
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(false);
      try {
        const result = await api.predictWhatIf(input, studentLevel, studentId);
        if (active) {
          if (result) {
  ...
  }, [input, studentLevel, studentId, isInitialized]);
